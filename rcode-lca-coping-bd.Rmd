...
## Enumeration: Estimate *K*-class models with 1 through 6 classes.

# --------------------------------------------------------------------------------------

**Example specific syntax that varies based on research context:**

- Within the `lapply()` function `1:6` indicates the number of *K*-class models to estimate.
- The measurement model item names for the coping strategy example are listed after the `categorical =` & `usevar =` statements. Note, that for Latent Profile Analysis (LPA) the `categorical =` statement should be removed so that items are estimated as continuous variables. 
...
**Generate a summary table of model fit information:**
...
This syntax produces a plot containing a series of LCA probability plot facets. This can be used to compare solutions across *K*-class models (*Figure A1*).
...
```{r}
ggsave(here("figures","FigA1_LCA_K1-6.png"), dpi=300, height=4, width=6, units="in")  
```
...
### Step 3 (part 1) - Estimate the unconditional model with logits from step 2. This model is estimated to check that the class proportions are approximately the same as in step 1. 
```{r}
...
## Step 3 (part 2) - Estimate the moderation model with covariates & distal outcomes. 
...
m_step3  <- mplusObject(
 ...
 "New (diff12 diff13
  ...
  MODELTEST =
  ### NOTE: Only a single Wald test can be conducted per model run. Therefore, this example requires running separate models for each omnibus test (e.g., 4 models; 2 outcomes and 2 slope coefficients). This can be done by commenting out all but one test and then making multiple input/output files. ###
...
# Generate figures associated with Coping Strategy study: 
...
## Plot distal outcomes grouped by class (*Figure 3*) CHECK ALL FIGURES FOR CORRESPONDENCE
```{r, eval=TRUE}
